 ...
 * Get list of upcoming IPOs
  ...
  async equityBhavcopy(date, folder = null) {
     ...
     */
    const url = `${this.archiveUrl}/content/cm/BhavCopy_NSE_CM_0_0_0_${date.toLocaleDateString('en-IN').replace(/\//g, '')}_F_0000.csv.zip`;
    return await this.__download(url, folder);
  }
  ...
  async deliveryBhavcopy(date, folder =null) {
     ...
     * @returns {Promise<string>}
     */
    const url = `${this.archiveUrl}/products/content/sec_bhavdata_full_${date.toLocaleDateString('en-IN').replace(/\//g, '')}.csv`;
    return await this.__download(url, folder);
  }

  async indicesBhavcopy(date, folder = null) {
    /**
     ...
     * @returns {Promise<string>}
     */
    const url = `${this.archiveUrl}/content/indices/ind_close_all_${date.toLocaleDateString('en-IN').replace(/\//g, '')}.csv`;
    return await this.__download(url, folder);
  }
  ...
  async fnoBhavcopy(date, folder = null) {
     ...
     * @returns {Promise<string>}
     */
    const url = `${this.archiveUrl}/content/fo/BhavCopy_NSE_FO_0_0_0_${date.toLocaleDateString('en-IN').replace(/\//g, '')}_F_0000.csv.zip`;
    return await this.__download(url, folder);
  }

  async pricebandReport(date, folder = null) {
    /**
     ...
     * @returns {Promise<string>}
     */
    const url = `${this.archiveUrl}/content/equities/sec_list_${date.toLocaleDateString('en-IN').replace(/\//g, '')}.csv`;
    return await this.__download(url, folder);
  }
  ...
  async prBhavcopy(date, folder = null) {
     ...
     * @returns {Promise<string>}
     */
    const url = `${this.archiveUrl}/archives/equities/bhavcopy/pr/PR${date.toLocaleDateString('en-IN').replace(/\//g, '').slice(2)}.zip`;
    return await this.__download(url, folder);
  }

  async __download(url, folder) {
    ...
    const downloadFile = async (attempt = 1) => {
      try {
        const response = await axios.get(url, { responseType: 'stream', timeout: 30000 }); // Increased timeout to 30 seconds

        const contentType = response.headers['content-type'];
        if (contentType && contentType.includes('text/html')) {
          throw new Error('NSE file is unavailable or not yet updated.');
        }else if (contentType && contentType.includes(undefined)){
          throw new Error('NSE file is unavailable or not yet updated.');
        }
